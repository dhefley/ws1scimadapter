...
*/
  ...
  */
  ...
  let auth = async (ctx, next) => { // authentication/authorization
    let [authType, authToken] = (ctx.request.header.authorization || '').split(' ') // [0] = 'Basic' or 'Bearer'
    authType = "Basic"
    ctx.request.header.authorization = authType + ' ' + authToken
    let unauth = (ctx) => {
  ...
  router.get(`/:baseEntity/ping`, async (ctx) => { // auth not required
    let tx = 'hello'
  ...
  router.get([`/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`,
    ...
    try {
      let data = await this[handle.getMethod](ctx.headers.authorization, ctx.params.baseEntity, id, ctx.query.attributes ? ctx.query.attributes : '')
  ...
  router.get([`/(|scim/)(Users|Groups|servicePlans)`,
    ...
    if (!ctx.query.filter) {
      ...
      try {
        let data = await this[handle.exploreMethod](ctx.headers.authorization, ctx.headers.authorization, ctx.params.baseEntity, ctx.query.attributes ? ctx.query.attributes : '', parseInt(ctx.query.startIndex), parseInt(ctx.query.count))
        let scimdata = data
      ...
      if (arrFilter.length > 2 && arrFilter[1] === 'eq') {
        if ((arrFilter[0] === handle.uniqueAttr || arrFilter[0] === 'id' || arrFilter[0] === 'externalId' || arrFilter[0] === 'members')) {
          ...
          let searchAnchor = arrFilter[0]
          logger.debug(`${gwName}[${pluginName}] [Get ${handle.description}] ${handle.uniqueAttr}=${identifier}`)
          logger.debug(`${gwName}[${pluginName}] calling "${handle.getMethod}" and awaiting result`)

          try {
            let data = await this[handle.getMethod](ctx.headers.authorization, ctx.params.baseEntity, identifier, searchAnchor, ctx.query.attributes ? ctx.query.attributes : '')
            if (!data) data = {}
          ...
          try {
            let data = await this[handle.inclusionMethod](ctx.headers.authorization, ctx.params.baseEntity, identifier, ctx.query.attributes)
            let scimdata = data
            scimdata = addResources(scimdata, ctx.query.startIndex)
            scimdata = addSchemas(scimdata, handle.description, isScimv2)
            ctx.body = scimdata
  ...
  router.post([`/(|scim/)(!${undefined}|Users|Groups)(|.json)(|.xml)`,
    ...
    try {
      let awreturn = await this[handle.createMethod](ctx.headers.authorization, ctx.params.baseEntity, scimdata)
      let location = `${ctx.request.href}/${jsonBody.externalId}`
      if (!jsonBody.meta) jsonBody.meta = {}
      jsonBody.meta.location = location
      jsonBody.id = jsonBody.externalId ? jsonBody.externalId : jsonBody.userName
      jsonBody.externalId = awreturn.body.uuid
  ...
  router.delete([`/(|scim/)(!${undefined}|Users|Groups)/:id`,
    ...
    try {
      await this[handle.deleteMethod](ctx.headers.authorization, ctx.params.baseEntity, id)
      ctx.status = 204
    } catch (err) {
      let e = jsonErr(config.scim.version, pluginName, ctx.status, err)
      ctx.status = 500
  ...
  router.patch([`/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`,
    ...
    if (strBody === '{}') {
      ...
      try {
        await this[handle.modifyMethod](ctx.headers.authorization, ctx.params.baseEntity, id, scimdata.members ? scimdata.members : scimdata)
        if (handle.modifyMethod === "modifyUser") {
          let location = ctx.request.href
          jsonBody.id = id
          delete jsonBody.password
          ctx.set('Location', location)
        ...
        } else if (handle.modifyMethod === "modifyGroupMembers"){
          ctx.status = 204
        }
...
ScimGateway.prototype.convertedScim = function convertedScim (obj) {
  ...
  for (let key in scimdata) {
    if (Array.isArray(scimdata[key]) && (scimdata[key].length > 0) && scimdata[key][0].type) { // exclude "none type" multivalue attributes (e.g groups and x509Certificates)
      ...
      delete scimdata[key]
    } else if (key.indexOf('schemas') > -1) {
      for (let zattribute in scimdata[key]) {
        if (!newMulti[zattribute]) {
          newMulti[zattribute] = scimdata[key][zattribute]
        }
      }
      delete scimdata[key]      
    }
...
let addSchemas = (data, obj, isScimv2) => {
  if (!isScimv2) {
    ...
    if (data.Resources) data.schemas = ['urn:ietf:params:scim:api:messages:2.0:ListResponse']
      ...
      data.schemas = ['urn:ietf:params:scim:schemas:core:2.0:User', 'urn:ietf:params:scim:schemas:extension:enterprise:2.0:User', 'urn:ietf:params:scim:schemas:extension:scimgateway:2.0:User']
    } else if (obj === 'Group') {
...
let convertedScim20 = (data) => {
  ...
  data.Operations.forEach(function (element, index) {
    ...
    if (element.path) {
      ...
      if (element.path.indexOf('schemas') > -1) {
        element.path = element.path.split(":").pop()
      }
      ...
      if ((element.op).toLowerCase() === 'replace' || (element.op).toLowerCase() === 'add') {
        groupMembers.operation = (element.op).toLowerCase()
